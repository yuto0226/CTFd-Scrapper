...
Examples:
  ...
  ctfdscraper -u https://ctf.example.com -s cookie --max-workers-chal 15
    ...
    # Required arguments
    parser.add_argument(
        "-u", "--url", required=True, help="CTFd instance URL (e.g., https://ctf.example.com)"
    )

    parser.add_argument(
        "-s", "--session", required=True, help="Session cookie value for authentication"
    )

    # Optional arguments
    parser.add_argument(
        "-n", "--name", help="CTF name (auto-detected from website if not specified)"
    )
    ...
    backup_group.add_argument("--no-chal", action="store_true", help="Skip challenges backup")

    backup_group.add_argument("--no-team", action="store_true", help="Skip teams backup")

    backup_group.add_argument("--no-user", action="store_true", help="Skip users backup")

    backup_group.add_argument("--no-scoreboard", action="store_true", help="Skip scoreboard backup")
    ...
    timeout_group.add_argument(
        "--api-timeout", type=int, default=15, help="API request timeout in seconds (default: 15)"
    )
